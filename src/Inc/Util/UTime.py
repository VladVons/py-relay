...
def DelayMiliSec(aTime: int) -> float:
    time.sleep(aTime / 1000.0)

def DelayMicroSec(aTime: int) -> float:
    time.sleep(aTime / 1000000.0)

def CharToSec(aChar: str, aValue: int) -> int:
    # 1H1M1S -> 3661
...
def CheckRange(aValue: int, aMin: int, aMax: int) -> bool:
    if (aMin > aMax):
